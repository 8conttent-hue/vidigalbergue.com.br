...
export default function PostsManager() {
    ...
    const handleDelete = async (path: string, sha: string, name: string) => {
        ...
        try {
            ...
            triggerToast(`Post "${name}" excluído!`, 'success');
        } catch (err: any) {
    ...
    const saveQuickEdit = async () => {
        ...
        try {
            ...
            triggerToast('Post atualizado com sucesso!', 'success');
            setEditingSha(null);
            fetchInitialData();
        } catch (e: any) {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 text-slate-400 bg-white rounded-3xl border border-slate-200">
            ...
            <p className="font-medium animate-pulse">Carregando posts...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-6 pb-32">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 px-6 rounded-2xl border border-slate-200 shadow-sm sticky top-0 z-40">
                <div>
                    <h2 className="text-lg font-bold text-slate-800">Posts do Blog</h2>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">{posts.length} posts no total</p>
                </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap gap-3 items-center">
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input type="text" placeholder="Buscar posts..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500" />
                </div>
            ...
            {paginated.length === 0 ? (
                <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-3xl p-16 flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-xl font-bold text-slate-700 mb-2">Nenhum post encontrado</h3>
                    <a href="/admin/posts/new" className="mt-4 bg-violet-600 text-white font-bold px-8 py-3 rounded-xl shadow-md hover:bg-violet-700 transition-colors">Criar primeiro post</a>
                </div>
            ) : (
