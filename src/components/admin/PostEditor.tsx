...
// Quill toolbar restrita a H2/H3 (sem H1 no body — H1 = título do post)
const quillModules = {
    toolbar: [
        [{ header: [2, 3, false] }],
...
export default function PostEditor({ filePath }: PostEditorProps) {
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        triggerToast('Processando e salvando post...', 'progress', 20);
        try {
            ...
            const res = await githubApi('write', targetPath, { content: markdown, sha: fileSha || undefined, message: `CMS: ${isEditing ? 'Edição' : 'Criação'} do post ${post.slug}` });
            if (res.sha) setFileSha(res.sha);
            setPendingUploads({});
            triggerToast('Post salvo com sucesso!', 'success', 100);
        ...
        <div className="max-w-5xl pb-32">
            <div className="flex items-center justify-between bg-white p-4 px-6 rounded-2xl border border-slate-200 shadow-sm mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">{isEditing ? 'Editar Post' : 'Novo Post'}</h2>
            ...
            <div className="flex gap-6 items-start">
                ...
                <div className="flex-1 min-w-0 space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        <label className={labelClass}>Título do Post *</label>
                        <input type="text" value={post.title} onChange={e => handleTitleChange(e.target.value)} className={inputClass} placeholder="Título do post..." />
                        <div className="mt-3">
                            <label className={labelClass}>Slug (URL) *</label>
                            <input type="text" value={post.slug} onChange={e => setPost(p => ({ ...p, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))} className={`${inputClass} font-mono text-xs`} placeholder="url-do-post" />
                        </div>
                        <div className="mt-3">
                            <label className={labelClass}>Descrição / Meta Description</label>
                            <textarea rows={2} value={post.description} onChange={e => setPost(p => ({ ...p, description: e.target.value }))} className={`${inputClass} resize-none`} placeholder="Breve descrição do post..." />
                        </div>
                    </div>

                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        <label className={labelClass}>Conteúdo do Post</label>
                        <p className="text-xs text-slate-400 mb-3">Use H2 para seções principais e H3 para subseções. Não use H1 (o título já é o H1).</p>
                        {isPreview ? (
